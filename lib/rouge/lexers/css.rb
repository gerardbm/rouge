    ...
    class CSS < RegexLexer
      ...
      def self.properties
        @properties ||= Set.new %w(
          ...
          fit fit-position flex flex-basis flex-direction flex-flow
          ...
          inline-box-align justify-content justify-items justify-self left
          ...
          margin margin-bottom margin-left margin-right margin-top mark
          mark-after mark-before marker-offset marks marquee-direction
          marquee-loop marquee-play-count marquee-speed marquee-style mask
          max-height max-width min-height min-width move-to nav-down
          ...
          src stress string-set suffix symbols syntax system tab-size
          ...
          voice-family voice-pitch voice-pitch-range voice-range voice-rate
      ...
      def self.builtins
        @builtins ||= Set.new %w(
          above absolute all always armenian aural auto auto-fill auto-fit
          avoid balance baseline behind below bidi-override blink block
          bold bolder border-box both bottom bottom capitalize center
          center-left center-right circle cjk-ideographic close-quote
          collapse column column-reverse condensed contain content
          content-box continuous cover crop cross crosshair cursive dashed
          decimal decimal-leading-zero default digits disc dotted double
          e-resize embed end expanded extra-condensed extra-expanded
          fantasy far-left far-right fast faster fill fixed flex flex-end
          flex-start georgian grid groove hebrew help hidden hide high
          higher hiragana hiragana-iroha icon inherit inline inline-flex
          inline-grid inline-table inset inside italic justify katakana
          katakana-iroha landscape large larger left left left-side
          leftwards level lighter line-through list-item loud low lower
          lower-alpha lower-greek lower-roman lowercase ltr medium
          message-box middle mix monospace n-resize narrower ne-resize
          no-close-quote no-open-quote no-repeat none normal nowrap
          nw-resize oblique once open-quote outset outside overline pointer
          portrait px relative repeat-x repeat-y ridge right right-side
          rightwards row row-reverse rtl s-resize sans-serif scale-down
          scroll se-resize semi-condensed semi-expanded separate serif show
          silent slow slower small-caps small-caption smaller soft solid
          space-aroun space-between space-evenly span spell-out square
          start static status-bar sticky stretch sub super sw-resize swap
          ...
          upper-roman uppercase visible w-resize wait wider wrap
          wrap-reverse x-fast x-high x-large x-loud x-low x-small x-soft
          xx-large xx-small yes
        )
      end
      ...
      def self.functions
        @functions ||= Set.new %w(
          ...
          env exp fit-content grayscale hsl hsla hue-rotate hwb hypot
          ...
          perspective polygon pow radial-gradient rect rem repeat
          ...
          sepia sign sin skew skewX skewY skewZ sqrt steps styleset
